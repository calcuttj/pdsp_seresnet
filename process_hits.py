...
class PDSPData:
  def __init__(self, filename, linked=False):
    ...
    if not linked:
      ...
      for k in self.keys:
        ...
        self.event_keys += [k]*len(np.array(self.h5in[f'{k}/events/event_id']))
    ...
    if not self.linked:
      ...
      for k in self.keys:
        if 'truth' in self.h5in[f'{k}'].keys():
          found_truth = True
          pdg += [i for i in np.array(self.h5in[f'{k}/truth/pdg'])]
  ...
  def make_plane(self, pid, pad2=False):
    plane = np.zeros((913, 480 if (pid == 2 and not pad2) else 800))
  ...
  def get_training_batches(self, batchsize=2):

    for i in range(ceil(self.nevents/batchsize)):
      #print('Batch', i)
      #print('\t', np.arange(self.nevents)[i*batchsize:(i+1)*batchsize])

      batch_events = np.arange(self.nevents)[i*batchsize:(i+1)*batchsize]
      nb = len(batch_events)
      plane_batch = np.zeros((nb, 3, 913, 800))
      truth_batch = np.zeros((nb, 4))
      for a, j in enumerate(batch_events):
        #print(j)
        self.get_event(j)
        plane_batch[a,0,:,:] = self.make_plane(0)
        plane_batch[a,1,:,:] = self.make_plane(1)
        plane_batch[a,2,:,160:-160] = self.make_plane(2)
